<?php
	...
	if(isset($_GET['text']) && isset($_GET['date']) && isset($_GET['time'])){
		deleteTask($_GET["text"], $_GET["date"], $_GET["time"], $cod);
		earnPoints($cod);
	}
	...
	if($count==0){
		...
		foreach ($all as $row) {
			...
			if($row['date']==$dateNow && $row['time']>$timeNow){
				array_push($today, $row['text'].",".$row["time"].",".$row["date"]);
			}elseif($row['date']>$dateNow && $days==1){
				array_push($tomorrow, $row['text'].",".$row["time"].",".$row["date"]);
			}elseif($row['date']>$dateNow && $days>1){
				array_push($someday, $row['text'].",".$row["time"].",".$row["date"]);
			}else{
		...
		foreach($today as $day){
			$arr = explode(",", $day);
			echo "<div class='task'><p>".$arr[0]." at ".substr($arr[1] , 0, 5)."</p><span class='hiddenSpan1'>".$arr[2]."</span><span class='hiddenSpan2'>".$arr[1]."</span><div class='checkTask'></div></div>";
		}
		echo "</div>";
